...
const bgLoop = new Audio ('sounds/bg_loop.flac');
...
let upAPressed = false;
let downYPressed = false;
...
// function to draw extendedInfo
function drawInfo(x, y, info) {
  ctx.fillStyle = '#fff';
...
function update() {
  ...
  // move the paddle for User 2
  if (upAPressed && user2.y > 0) {
    user2.y -= 8;
  } else if (downYPressed && (user2.y < canvas.height - user1.height)) {
    user2.y += 8;
  }
...
function render() {
  ...
  // draw user paddle
  drawInfo(canvas.width -200, canvas.height - 10 , 'Speed: ' + ball.speed);
  // draw ai score
